...
if (_anti_burper_device	!="") then {
	...
	[_object_anom_burp, _anti_burper_device, _killrange] execVM "z\root_anomalies\addons\main\scripts\Burper\burper_remove.sqf";
};
...
if (_device_detector !="") then {
	...
	if (_isaipanic) then {
		[_object_anom_burp] execVM "z\root_anomalies\addons\main\scripts\Burper\burper_ai_avoid.sqf";
	};
	if (_isaipanic) then {
		[_object_anom_burp] execVM "z\root_anomalies\addons\main\scripts\Burper\burper_ai_avoid.sqf";
	};
	...
	if (_isaipanic) then {
		[_object_anom_burp] execVM "z\root_anomalies\addons\main\scripts\Burper\burper_ai_avoid_vizible.sqf";
	};
};

if (_damage_protect != "") then {obj_prot_burper = _damage_protect; publicVariable "obj_prot_burper"};

[_object_anom_burp, _burper_radius, _vehicle_allowed, _anti_burper_device] execVM "z\root_anomalies\addons\main\scripts\Burper\burper_damage_trap.sqf";
[[_object_anom_burp], "z\root_anomalies\addons\main\scripts\Burper\burper_sfx.sqf"] remoteExec ["execVM", 0, true];
