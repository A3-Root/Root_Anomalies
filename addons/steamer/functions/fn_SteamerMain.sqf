...
[_steamer] remoteExec ["Root_fnc_SteamerVoice", 0, true];
...
while {alive _steamer} do {
    ...
    while {(!isNil "_tgt") && {alive _steamer}} do {
        ...
        [getPosATL _tgt, selectRandom [false, true, false, false]] remoteExec ["Root_fnc_SteamerBurst"];

        {
            private _u = _x;
            if (_u isKindOf "CAManBase") then {
                [_u, _damage, _bodyParts selectRandomWeighted _weights, selectRandom ["backblast", "bullet", "explosive", "grenade"]] call Root_fnc_applyDamage;
            };
            if ((_u isKindOf "LandVehicle") || {_u isKindOf "Air"}) then {
                [_u, _damage] call _killVehicle;
            };
            if (isPlayer _u) then {[_burstPoz, _u] remoteExec ["Root_fnc_SteamerRagdoll", _u]} else {[_burstPoz, _u] spawn Root_fnc_SteamerRagdoll};
        } forEach _blowUnits;
...
[getPosATL _steamer] remoteExec ["Root_fnc_SteamerEnd", [0, -2] select isDedicated];
...
{_x setDamage [_deathDamage, false]} forEach nearestObjects [position _steamer, ["BUILDING", "HOUSE", "CHURCH", "CHAPEL", "FUELSTATION", "HOSPITAL", "RUIN", "BUNKER", "Land_fs_roof_F", "Land_TTowerBig_2_F", "Land_TTowerBig_1_F", "Lamps_base_F", "PowerLines_base_F", "PowerLines_Small_base_F", "Land_LampStreet_small_F"], 20, false];
{
    if (typeOf _x != "VirtualCurator_F") then {
        [_x, _deathDamage, _bodyParts selectRandomWeighted _weights, selectRandom ["backblast", "bullet", "explosive", "grenade"]] call Root_fnc_applyDamage;
    };
