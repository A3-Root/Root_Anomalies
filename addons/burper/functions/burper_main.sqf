...
if (_device_detector !="") then {
	...
	if (_isaipanic) then {
		[_object_anom_burp] execVM "burper_ai_avoid.sqf";
	};
	if (_isaipanic) then {
		[_object_anom_burp] execVM "burper_ai_avoid.sqf";
	};
	...
	if (_isaipanic) then {
		[_object_anom_burp] execVM "burper_ai_avoid_vizible.sqf";
	};
};

if !(_damage_protect) then {obj_prot_burper = _damage_protect; publicVariable "obj_prot_burper"};

[_object_anom_burp, _burper_radius, _vehicle_allowed, _anti_burper_device] execVM "burper_damage_trap.sqf";
[[_object_anom_burp], "burper_sfx.sqf"] remoteExec ["execVM", 0, true];
