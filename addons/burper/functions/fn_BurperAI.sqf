 ...
 * Description: AI panic loop used when the Burper requires a detection device -
 *              only AI carrying the detector flee.
 *
 * Arguments:
...
while {alive _obj} do {
    ...
    private _units = (position _obj) nearEntities ["Man", 80];
    {
        private _unit = _x;
