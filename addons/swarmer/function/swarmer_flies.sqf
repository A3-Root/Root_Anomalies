...
{flow_back = (_this vectorFromTo [(getPosATL swarmer_public # 0)+random (selectRandom [1,-1]),(getPosATL swarmer_public # 1)+random (selectRandom [1,-1]),random 2])}
	...
	if (atak_swarmer) then {
		...
		flow_back = (_this vectorFromTo [getPosATL _tgt_sw # 0,getPosATL _tgt_sw # 1,0.5+random 1]) vectorMultiply (3+random 5);
		if (_this vectorDistance (getPosATL _tgt_sw) < 1) then {atinge=true;};
	} 
	else {flow_back = (_this vectorFromTo [(getPosATL swarmer_public # 0)+random 2,(getPosATL swarmer_public # 1)+random 2,0.5+random 1]) vectorMultiply (3+random 5)};
};
