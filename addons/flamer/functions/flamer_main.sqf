...
#include "\z\root_anomalies\addons\flamer\functions\flamer_functions.hpp"
...
while {alive _flamer} do 
{
	...
	while {(!isNil "_tgt_flamer") && {(alive _flamer) && ((_flamer distance getMarkerPos _poz_orig_sc) < _teritoriu)}} do 
	{
		...
		else 
		{
			...
			} forEach (_nearflamer-[_flamer]); [[_flamer],"\z\root_anomalies\addons\flamer\functions\flamer_jump_SFX.sqf"] remoteExec ["execvm"]; [_flamer,_tgt_flamer,_cap_flamer,_damage_flamer] spawn fnc_jump_flamer};
		uiSleep _recharge_delay;
		_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities [["CAManBase","LandVehicle"],5];
		{
			_bodyPart = ["Head", "RightLeg", "LeftArm", "Body", "LeftLeg", "RightArm"] selectRandomWeighted [0.47,0.69,0.59,0.55,0.61,0.58];
		...
		{_flamer setVariable ["atk",true]; [_flamer,_tgt_flamer,_damage_flamer] spawn fnc_attk_flamer; uiSleep 0.5; [[_tgt_flamer],"\z\root_anomalies\addons\flamer\functions\flamer_atk_SFX.sqf"] remoteExec ["execvm"]};
		uiSleep _recharge_delay;
		_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities [["CAManBase","LandVehicle"],5];
		{
			_bodyPart = ["Head", "RightLeg", "LeftArm", "Body", "LeftLeg", "RightArm"] selectRandomWeighted [0.47,0.69,0.59,0.55,0.61,0.58];
