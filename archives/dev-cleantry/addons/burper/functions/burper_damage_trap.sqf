		...
		if (count _list_units_in_range > 0) then {
			{
				if ((typeOf _x == "VirtualCurator_F") or ([_x, obj_prot_burper] call BIS_fnc_hasItem)) then {
					...
					[_oase] remoteExec ["Root_fnc_BurperSplash", [0, -2] select isDedicated];
					...
					[_object_burp_damage, ["blood_splash", 100]] remoteExec ["say3D"];
					uiSleep 0.3;
					[_balta_sange, ["bones_drop", 100]] remoteExec ["say3D"];
				};
